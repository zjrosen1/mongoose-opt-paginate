...
function decode(encoded) {
	try{
		return mongoose.Types.ObjectId(encoded);
	}catch (e){
		return null;
	}
}

exports.setQueryParams = function setQueryParams(req, reply, defaultPageSize, maxPageSize) {
	...
	if (req.query.before) {
		...
		if(req.query.before == null){
			reply(Boom.badData({message: 'before should be valid object id'}));
		}
	}
	...
	if (req.query.after) {
		...
		if(req.query.after == null){
			reply(Boom.badData({message: 'after should be valid object id'}));
		}
	}
};
...
exports.href = function href(req) {
	return function(prev, params) {
		...
		return url.parse(req.url.path).pathname + '?' + querystring.stringify(query);
	};
};
